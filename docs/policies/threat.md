...
- Intrusion Detection
- Threat Intelligence
description: In order to preserve the integrity of data that Tessera stores, processes,
  or transmits for Customers, Tessera implements strong intrusion detection tools
...
In order to preserve the integrity of data that Tessera stores, processes, or
transmits for Customers, Tessera implements strong intrusion detection tools
...
3. All systems are to only be used for Tessera business needs.
...
* Tessera leverages AWS GuardDuty and Cisco Meraki for network security of its on-premise
  environments.
...
* Tessera IT manager is responsible for configuring the firewall and IDS/IPS
  rules and review the configuration as least quarterly.

#### Intrusion Detection in AWS Cloud Environments

Tessera implemented a real-time threat detection solution by
...
Tessera is an active member of the [National Health Information Sharing and
Analysis Center (NH-ISAC)](https://nhisac.org/).

Tessera Security team is subscribed to receive threat alerts from NH-ISAC.
