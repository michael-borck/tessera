...
description: Tessera standardises and automates configuration management through
...
Tessera standardises and automates configuration management through the use of
...
configure all Tessera systems according to established and tested policies,
...
5. Tooling is used to generate an up to date system inventory.
    ...
    * All systems are classified and labeled based on the data they store or
      process, according to [Tessera data classification
    ...
    * IT assets database is used to generate the diagrams and asset lists
      required by the Risk Assessment phase of Tessera's
      [Risk Management procedures](risk_mgmt.md)
    * Tessera Change Management process ensures that all asset inventory
...
6. Tessera uses the [Security Technical Implementation Guides
...
7. All IT assets in Tessera have time synchronised to a single
...
7. The new system is fully promoted into production upon successful verification
   against corresponding Tessera standards and change request approvals.
...
Tessera maintains a single Organisation in AWS, maintained in a
...
records for Tessera root domain, using AWS Route53 service. DNS
...
Tessera AWS environments and infrastructure are managed as code.
...
`Tessera-master`.  The creation and provisioning of a new account
...
The Tessera Continuous Delivery Pipeline automates creation and
...
Tessera uses automated tooling to ensure systems are up-to-date
...
Tessera follows a "cattle-vs-pets" methodology to keep the
...
Tessera requires auto-update for security patches to be enabled for
...
which implements the Tessera Change Management workflow, using the
...
* Additional approver(s) may be added depending on the impacted component(s).
  For example,
    ...
    - the DevOps Lead is added as an approver for changes to `aws-Tessera-infra`
      account in AWS.
...
* Additional details are required for a code deploy, including:
    ...
    - Target environment (e.g. `aws-Tessera-infra`, `aws-Tessera-prod-us`, `datacenter-hq`)
